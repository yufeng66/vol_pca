...
- `factors.py` — PCA on daily pillar Δσ, **correlation-based by default**: standardizing stops high-variance short-TTM pillars from dominating, which roughly halves low-k truncation error vs covariance PCA (k=1 R² 0.85 vs 0.75). Exposures `E_k = (bucket_vega·scale)·L_k`, scores `f_k`, estimate `= bucket_vega·μ + Σ E_k f_k`.
